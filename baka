-- ================================================
-- BAY ORBIT v19.0 - NO STICK (ÄÃƒ XOÃ GHIM)
-- ================================================
...
-- KEY SYSTEM (GITHUB)
-- ================================================
local KEY_URL = "https://raw.githubusercontent.com/nicodeptrai0-ctrl/B-Minh-v-i/main/0396196227keyx.txt"
	...
	verifyBtn.MouseButton1Click:Connect(function()
		...
		if s and r then
			...
			for k in string.gmatch(r, "[^\r\n]+") do
				...
				local keyPart = parts[1]
				local datePart = parts[2]
				keyPart = keyPart and keyPart:match("^%s*(.-)%s*$")
				local cleanInput = input:match("^%s*(.-)%s*$")
				
				if keyPart == cleanInput then 
					if datePart then
						datePart = datePart:match("^%s*(.-)%s*$")
...
-- ================================================
-- CODE CHIẾN ĐẤU (SAU KHI XÁC NHẬN KEY)
-- ================================================
...
local BF_DELAY = 0.1 -- thá»i gian thá»±c hiá»‡n Black Flash trÆ°á»›c khi xoay Orbit (giÃ¢y)
local BF_DELAY_RATIO = 0.73
local FACE_TARGET_AFTER = 0.5 -- thá»i gian hÆ°á»›ng máº·t vÃ o Ä‘á»‘i thá»§ SAU khi orbit xong (giÃ¢y)
...
-- ================== BLACK FLASH ==================
local function blackflash(tRoot)
...
local function stopOrbit()
	...
	orbiting = false
	local lastTarget = targetRoot -- lÆ°u target trÆ°á»›c khi xoÃ¡
	targetRoot = nil
	...
	-- Giá»¯ hÆ°á»›ng máº·t vÃ o Ä‘á»‘i thá»§ sau orbit
...
local function startOrbit()
	...
	orbitConn = RunService.RenderStepped:Connect(function(dt)
		...
		if elapsed > Config.MAX_ORBIT_TIME + 0.1 then stopOrbit() return end
		...
		smoothedAngle = smoothedAngle + orbitDirection * rad(Config.ANG_SPEED_DEG) * dt * 1.12
		...
		local progress = clamp(elapsed / Config.MAX_ORBIT_TIME, 0, 1)
		...
		if toGoal.Magnitude > 0.5 then
			local targetVel = toGoal.Unit * (Config.MOVE_SPEED * clamp(toGoal.Magnitude / 6, 0.4, 1.1))
...
local function createConfigMenu()
	...
	-- ===== HELPER: INPUT ROW =====
	local function addInputRow(parent, labelText, configKey, order)
		local row = Instance.new("Frame")
		...
		tb.Text = tostring(Config[configKey])
		...
		tb.FocusLost:Connect(function()
			...
			if num then Config[configKey] = num; tb.Text = tostring(num)
			else tb.Text = tostring(Config[configKey]) end
		end)
		return tb
	end
	...
	addInfoRow(pageGeneral, "Hotkey Orbit", "C", 3)
	...
	addToggle(pageFeatures, "🔥 GOD Black Flash", false, 2, function(state)
		godBFEnabled = state
	end)

	addToggle(pageFeatures, "🎯 Aimbot", false, 3, function(state)
	...
	addSectionTitle(pageFeatures, "👁️ Visuals", 5)

	addToggle(pageFeatures, "👁️ ESP (Box/Name/Health)", false, 6, function(state)
		espEnabled = state
	end)
	...
	-- PAGE: CONFIG
	-- =========================================
	addSectionTitle(pageConfig, "🔄 Orbit Settings", 1)

	local tbSpeed = addInputRow(pageConfig, "Speed (deg/s)", "ANG_SPEED_DEG", 2)
	local tbTime = addInputRow(pageConfig, "Orbit Time (s)", "MAX_ORBIT_TIME", 3)
	local tbMove = addInputRow(pageConfig, "Move Speed", "MOVE_SPEED", 4)

	-- divider
	local cDiv = Instance.new("Frame")
	cDiv.Size = UDim2.new(1, 0, 0, 1)
	cDiv.BackgroundColor3 = DIVIDER
	cDiv.BorderSizePixel = 0
	cDiv.LayoutOrder = 5
	cDiv.Parent = pageConfig
	...
	resetBtn.MouseButton1Click:Connect(function()
		Config.ANG_SPEED_DEG = 1000; Config.MAX_ORBIT_TIME = 0.35; Config.MOVE_SPEED = 100
		tbSpeed.Text = "1000"; tbTime.Text = "0.35"; tbMove.Text = "100"
	end)
...
UIS.InputBegan:Connect(function(input, gameProcessed)
	...
	if input.KeyCode == Enum.KeyCode.C then
		...
		if orbiting then
			...
			-- 1. Tung Black Flash ngay láº­p tá»©c (chá»‰ khi GOD BF báº­t)
			if godBFEnabled then
				task.spawn(blackflash, tRoot)
			end
			
			-- 2. Äá»£i thá»i gian delay tá»« biáº¿n BF_DELAY
			task.spawn(function()
				task.wait(BF_DELAY)
				-- 3. Báº¯t Ä‘áº§u xoay Orbit
				if enabled then
					startOrbit()
				end
			end)
...
print("SIDE DASH ORBIT v19.0 + BLACK FLASH READY (NO STICK)")
print("C = Orbit(smooth) + BF(auto) | Insert = ON/OFF")
